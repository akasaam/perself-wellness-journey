...
const HeroSection: React.FC<HeroSectionProps> = ({
  ...
  return (
    <section 
      className={cn(
        "relative py-24 md:py-32 flex items-center min-h-[400px]", 
            ...
            {secondaryCtaText && secondaryCtaLink && (
              <Link to={secondaryCtaLink}>
                <Button 
                  variant="outline" 
                  size="lg" 
                  className={backgroundImage ? "bg-white/10 backdrop-blur-sm text-white hover:bg-white/20" : "text-gray-800"}
                >
                  {secondaryCtaText}
                </Button>
              </Link>
